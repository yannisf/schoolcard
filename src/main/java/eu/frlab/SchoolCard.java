...
public class SchoolCard {
    ...
    private void execute(ParsedOptions parsedOptions, CredentialsDataModel credentials, KidDataModel kid) throws InterruptedException {
        ...
        if (parsedOptions.isVerbose()) LOG.info("Άνοιγμα σελίδας edupass.gov.gr");
        String url = "https://edupass.gov.gr/";
        driver.manage().window().maximize();
        driver.get(url);

        if (parsedOptions.isVerbose()) LOG.info("Ας ξεκινήσουμε");
        driver.findElement(By.xpath("//span[text()[contains(.,'Ξεκινήστε εδώ')]]")).click();
        Thread.sleep(customWait);

        if (parsedOptions.isVerbose())  LOG.info("Επιλογή εκπαιδευτικής βαθμίδας");
        ...
        if (parsedOptions.isVerbose()) LOG.info("Επιλογή σχολικής κάρτας");
        ...
        if (parsedOptions.isVerbose()) LOG.info("Αποδοχή cookies");
        By acceptCookiesControl = By.xpath("//button[span[text()[contains(.,'Αποδοχή')]]]");
        driver.findElement(acceptCookiesControl).click();
        Thread.sleep(customWait);

        if (parsedOptions.isVerbose()) LOG.info("Αυθεντικοποίηση");
        driver.findElement(By.cssSelector("a[href='/api/login/']")).click();

        if (parsedOptions.isVerbose()) LOG.info("Επιλογή υπηρεσίας αυθεντικοποίησης (ΓΓΠΣΔΔ)");
        driver.findElement(By.xpath("//button[span[text()[contains(.,'ΓΓΠΣΔΔ')]]]")).click();

        if (parsedOptions.isVerbose()) LOG.info("Εισαγωγή διαπιστευτηρίων");
        ...
        if (parsedOptions.isVerbose()) LOG.info("Ανασκόπηση δεδομένων ΓΓΠΣΔΔ");
        try {
            driver.findElement(By.id("btn-submit")).click();
        } catch (Exception ignored) {
            if (parsedOptions.isVerbose()) LOG.info("Ανασκόπηση δεδομένων ΓΓΠΣΔΔ (παραλείφθηκε)");
        }

        if (parsedOptions.isVerbose()) LOG.info("Ανασκόπηση προσωπικών δεδομένων");
        ...
        if (parsedOptions.isVerbose()) LOG.info("Εισαγωγή προσωπικών δεδομένων μαθητή");
